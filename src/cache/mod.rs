...
// SQLite LLM response cache, and migration helpers.
//
// Before RC-CLEANUP-B these lived as four top-level siblings
// (`src/cache.rs`, `src/cache_layer.rs`, `src/response_cache.rs`,
// `src/cache_migrate.rs`). They've been consolidated under
// `crate::cache::*` without behaviour changes; this module groups the
// four files and re-exports the `audit` submodule's public API at the
// module root so callers that imported from the previous
// `crate::cache::*` path keep working unchanged.
//
